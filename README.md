...
The type of any object can be checked using `obj_is_type()`,
`obj_inspect_type()`, and `obj_assert_type()`:
...
dialect for setting type constraints on parts of an object using `on()`
selector functions, `has()`, and `has_relation()`:
...
trait compatible with `has_relation()`. For example, `na_string()`
requires that arguments `x` and `na_to` are either the same size or are
length-1, using the `recyclable()` trait.
